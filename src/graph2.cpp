...
void Graph2::display() {
	...
	for(int vertex = 0; vertex < nbVertex; vertex++){
		...
		int v1;
		int v2;
		if (matrix[vertex].first != -1) {val++; v1 = matrix[vertex].first;}else{v1 = -1;}
		if (matrix[vertex].second != -1) {val++; v2 = matrix[vertex].first;}else{v2 = -1;}
...
Vertex Graph2::getDegree(Vertex vertex){
	int val = 0;
	if (matrix[vertex].first != -1) {val++;}
	if (matrix[vertex].second != -1) {val++;}
	return val;
}
...
void Graph2::toDegree2(vector<std::pair<Vertex, bool>> list, GraphNO &g) {
    initEmptyGraph(g.getNbVertices());

    for (Vertex v = 0, int index = 0; v < g.getNbVertices(); v++) {
        if (v == list[index].first && !list[index].second) {
            continue;
		}

		voisins = g.getNeighbors(v);
